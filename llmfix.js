...
void async function llmUI() {
  ...
  async function zfetchArrayBuffer() {
    try {
      return await fetchArrayBuffer(...arguments);
    } catch (e) {
      return e.message;
    }
  }
          ...
          let doPump = setInterval(function pump() {
            if (pumpBuffer.length > 0) {
              ...
              if (message.includes('[/INST]')) {
                message = message.split('[/INST]')[1].split('</s>')[0];
              }
              if (message.includes('/INST]')) {
                message = message.split('/INST]')[1].split('</s>')[0];
              }
              if (message.includes('[INST]')) {
                message = message.split('[INST]')[0];
              }
  ...
  globalThis.zdecode = function() {
    return decoder.zdecode(...arguments);
  }
